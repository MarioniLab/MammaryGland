...
\textbf{Single cell RNA sequencing identifies 8 clusters of mammary epithelial cells}
...
Notably, some clusters represent the same putative cell type at different developmental time points (indicated by the NP, G, L or PI suffix) and were thus grouped together (\autoref{F1}c, \autoref{F1}d).
Based on the expression of \textit{Krt18,} \textit{Krt8, Krt5, Krt14} and \textit{Acta2} we noted that 5 clusters show a luminal profile (Lp, Hsp, Hsd, Avp \& Avd) and 3 a basal profile (Bsl, Myo \& Prc) (\autoref{F1}D, Supplementary Fig 2b and 2c).
Avd and Avp express high levels of beta-casein, \textit{Csn2}, and are exclusively composed of cells from the gestation and lactation time-points, suggesting that these cells are secretory alveolar cells (\autoref{F1}d).
...
The second, smaller group of genes that is switched on during the late stages of differentiation was enriched for genes involved in fatty-acid transport and lipid homeostasis (including e.g. \textit{Olah}, \textit{Cd36}, \textit{Wap} and \textit{Lalba}).
...
Genes with increasing expression are involved in hormone metabolic processes and regulation of morphogenesis of an epithelium (e.g. \textit{Vdr}, \textit{Esr1}, \textit{Pgr} and \textit{Msx2}).
...
Interestingly, the effect of parity is most pronounced for the less differentiated and more progenitor-like cell types (Lp-PI, \autoref{F1}c, Supplementary Fig 6a).
...
Notably, some of the genes that are involved in the immune response are known regulators of the involution process such as \textit{Ctsc}, \textit{Tgfb3} or \textit{Mfge8}.
...
\comment{Doesn't fit: Of note, the genes of the casein locus (\textit{Csn2}, \textit{Csn1s1}, \textit{Csn1s2a}, \textit{Csn3}) have previously been reported to be up-regulated in the parous gland, most likely due to changes in chromatin accessibility\autocite{Dos2015,Rijnkels2013}.
